...
public class FlockUnit : MonoBehaviour
{
    ...
    private Vector3 currentObstacleAvoidanceVctor;
    private float speed;
  ...
  public void MoveUnit()
    ...
    private Vector3 CalculateObstacleVector()
    {
        ...
        {
            obstacleVector = FindBestDirectionToAvoidObstacles();
        }
        else
        {
            currentObstacleAvoidanceVctor = Vector3.zero;
        }
        return obstacleVector;

    }

    private Vector3 FindBestDirectionToAvoidObstacles()
    {
        if(currentObstacleAvoidanceVctor != Vector3.zero)
        {
            RaycastHit hit;
            if(!Physics.Raycast(myTransform.position, myTransform.forward, out hit, assignedFlock.obstacleDistance, obstacleMask))
            {
                return currentObstacleAvoidanceVctor;
            }
        }
        ...
        for(int i = 0; i < directionsToCheckWhenAvoidingObstacles.Length; i++)
        {
            ...
            {
                selectedDirection = currentDirection;
                currentObstacleAvoidanceVctor = currentDirection.normalized;
